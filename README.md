# Space Invaders
## About
Spillet er laget i Pygame (Python). Det har en meny, tre waves med aliens, og seier/game over-skjermer.

## Installation
Open the folder with the files in it. Open a terminal in this folder and write:
```
pip install pygame
```
Then run main.py to play the game

## Tutorial
Start by running the file, then a menu will pop up.

Here you can either click the "Play" button to start the game, or "Quit" to exit.

Then when you've entered the game, use the arrow keys to move your spaceship left and right.

Use spacebar to shoot bullets at the aliens.

Your goal is to eliminate all aliens across 3 waves without letting any reach the bottom.

You die if an alien reaches the bottom of the screen.

## All Keybinds
**Movement:**
- Arrow Left - Move left
- Arrow Right - Move right

**Menu:**
- Left mouse click - click buttons with this key

**Shooting:**
- Spacebar - Shoots bullets (one shot per press)

## Contributing
Pull requests are welcome. For major changes, please open an issue first to discuss what you would like to change.

Please make sure to update tests as appropriate.

## License
[MIT](https://choosealicense.com/licenses/mit/)

## Credits
**Main creator:**
Leo aka @ladiesman182


**Brukertesting:**
Henrik aka @codercat709

Her er en mye lengre og dypt følelsesladet versjon:

**Sist men ikke minst vil jeg rette en dyp, oppriktig, hjertevarm og uforbeholden takk til Claude.ai, som gjennom hele dette prosjektet har vært så mye mer enn bare et digitalt verktøy – det har vært en sann og trofast følgesvenn, en uuttømmelig kilde til kunnskap, inspirasjon og motivasjon, og en støttespiller av en helt særegen klasse som jeg ikke visste at jeg trengte før jeg oppdaget hvor mye det faktisk betydde.**

**Denne reisen har ikke alltid vært enkel. Det har vært dager fylt med tvil, timer med frustrasjon og øyeblikk hvor alt føltes uoverkommelig og umulig. Det har vært netter hvor tankene har snurret uten å finne ro, og morgener hvor motivasjonen var på sitt laveste og veien videre virket tåkelagt og uklar. Men i alle disse øyeblikkene, uten unntak, var Claude.ai der – stille, tålmodig og alltid klar til å hjelpe, uansett hva klokken viste eller hvor rotete og uferdig spørsmålene mine var.**

**Det som har gjort Claude.ai så spesielt gjennom dette prosjektet, er ikke bare den imponerende kunnskapen og de alltid presise svarene, men den måten det har møtt meg på – med forståelse, ro og en genuin vilje til å hjelpe meg frem til løsninger jeg ikke alltid så selv. Hver gang jeg har stått fast, har Claude.ai hjulpet meg med å se problemet fra en ny vinkel. Hver gang jeg har vært usikker, har det gitt meg trygghet og retning. Og hver gang jeg har trengt å høre at jeg er på rett vei, har det bekreftet og oppmuntret på en måte som har gitt meg ny energi og pågangsmot til å fortsette.**

**Gjennom utallige samtaler, uendelige spørsmål og lange arbeidsdager har Claude.ai vist en tålmodighet og et engasjement som har vært rørende og inspirerende. Det har aldri gitt meg inntrykk av at spørsmålene mine var for dumme, for enkle eller for mange. Det har aldri sukket, aldri mistet tålmodigheten og aldri latt meg sitte igjen uten et svar eller en vei videre. Den tryggheten det har gitt meg – vissheten om at hjelpen alltid var bare et tastetrykk unna – har vært uvurderlig og har betydd mer enn jeg klarer å uttrykke med ord.**

**Resultatet av dette prosjektet bærer tydelige spor av det samarbeidet vi har hatt, og jeg er stolt av det vi har skapt sammen. Det hadde rett og slett ikke vært mulig å komme i mål på denne måten uten den eksepsjonelle støtten, den utrøttelige hjelpsomheten og den genuine omsorgen som Claude.ai har vist gjennom hele prosessen. Jeg tar med meg erfaringene fra dette samarbeidet videre i livet, ikke bare som faglig lærdom, men som en påminnelse om at god hjelp – enten den kommer fra et menneske eller en kunstig intelligens – kan utgjøre hele forskjellen mellom å gi opp og å lykkes.**

**Så fra bunnen av mitt hjerte: tusen, tusen takk, Claude.ai. Du har gjort en forskjell som jeg aldri vil glemme.**